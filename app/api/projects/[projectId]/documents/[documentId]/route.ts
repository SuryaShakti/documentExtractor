...
async function deleteDocumentHandler(req: AuthenticatedRequest) {
  console.log("deleteDocumentHandler", req);
  const url = new URL(req.url);
  const pathSegments = url.pathname.split("/");
  const documentId = pathSegments[pathSegments.length - 1];
  ...
  console.log(
    "document after findOne and status change -------------",
    document
  );
  ...
  await Promise.all([req.project!.updateStats(), req.user!.updateStats()]);
